...
class Workers extends ADMIN_Controller {
    ...
    public function __construct ()
    {
        ...
        $this->init($this->page);
     #   $this->init($this->page2);
    }

    public function index()
    {
        $count = $this->model->getCount(['access !=' => 'user']);
        ...
        $this->data['items'] = $this->model->getItems(['access !=' => 'user'], 'login ASC', $pagination['per_page'], $pagination['offset']);
    ...
    public function create()
    {
        ...
        if($_POST)
        {
            try
            {
                ...
                set_flash('result', action_result('success', fa5s('check mr5').' Работник <strong>"'.$insert['login'].'"</strong> успешно добавлен!', true));
    ...
    public function edit()
    {
        ...
        if(empty($item))
        {
            set_flash('result', action_result('error', fa5s('exclamation-triangle mr5').' Работник не найден!', true));
            redirect('admin/'.$this->page);
        }
        ...
        if($_POST)
        {
            try
            {
                ...
                set_flash('result', action_result('success', fa5s('check mr5').' Работник <strong>"'.$insert['login'].'"</strong> успешно обновлен!', true));
    ...
    public function delete()
    {
        ...
        try
        {
            if($_POST && $this->input->post('delete') == 'delete' && $id != $this->data['userid'])
            {
                $item = $this->model->getItem('id', $id);
                if(empty($item))
                    throw new Exception(fa5s('exclamation-triangle mr5').' Работник не найден!');
        ...
        if($error)
        {
            ...
            set_flash('result', action_result('success', fa5r('trash-alt mr5').' Работник <strong>"'.$item['login'].'"</strong> успешно удален!', true));
        }
